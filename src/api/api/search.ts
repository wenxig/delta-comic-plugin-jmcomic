...
export namespace _jmApiSearch.utils {
  ...
  export const byKeyword = PromiseContent.fromAsyncFunction((searchQuery: string, order: JmType.SortType = "", page = 1, signal?: AbortSignal) =>
    jmStore.api.value!.get<JmType.search.ByKeyword>('/search', { signal, params: { search_query: (searchQuery), page, o: order } }).then(v => v.content.map(createCommonToUniItem)))
  ...
  export const byCategory = PromiseContent.fromAsyncFunction((c: string, order: JmType.SortType = '', page = 1, signal?: AbortSignal) =>
    jmStore.api.value!.get<JmType.search.ByCategory>('/categories/filter', { signal, params: { c: (c), page, o: order } }).then(v => v.content.map(createCommonToUniItem)))
...
export namespace _jmApiSearch {
  ...
  export const getPromote = PromiseContent.fromAsyncFunction((signal?: AbortSignal) =>
    jmStore.api.value!.get<JmType.search.RawPromote[]>(`/promote`, { signal, params: { page: 1 } }))
  export const getPromoteItem = PromiseContent.fromAsyncFunction((id: number, page = 1, signal?: AbortSignal) =>
    jmStore.api.value!.get<JmType.search.PromoteItem>(`/promote_list`, { signal, params: { page, id } }).then(v => v.list.map(createCommonToUniItem)))
  ...
  export const getWeekBestComic = PromiseContent.fromAsyncFunction((id: number, type: string, signal?: AbortSignal) =>
    jmStore.api.value!.get<JmType.search.WeekBestItem>(`/week/filter`, { signal, params: { type, id } }).then(v => v.list.map(createCommonToUniItem)))
