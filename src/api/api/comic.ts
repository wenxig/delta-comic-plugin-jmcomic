...
export namespace _jmApiComic {
  ...
  export const getComic = PromiseContent.fromAsyncFunction(async (id: string, signal?: AbortSignal) => createFullToUniItem(await jmStore.api.get<JmType.comic.RawFullComic>(`/album?id=${id}`, { signal })))
  ...
  export const getComicPages = PromiseContent.fromAsyncFunction(async (id: string, signal?: AbortSignal) => {
    ...
    else var _chapter = await jmStore.api.get<JmType.comic.RawLessComic>(`/chapter?id=${id}`, { signal })
  ...
  export const likeComic = PromiseContent.fromAsyncFunction((id: string, signal?: AbortSignal) => jmStore.api.postForm('/like', { id }, { signal }))
  export const favouriteComic = PromiseContent.fromAsyncFunction((aid: string, signal?: AbortSignal) => jmStore.api.postForm<{
    status: string,
    msg: string,
    type: 'add' | 'remove'
  }>('/favorite', { aid }, { signal }))
